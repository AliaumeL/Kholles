...
\exercice[Développement limité d'une suite récurrente]
\begin{center}
...
\item En utilisant ce résultat, montrer que si la suite définie par $u_n = n^c$
    est entière pour tout $n$ alors $c \in \mb{N}$.

    \indication{Quelle est l'expression de la dérivée $n$-ème de $f : x \mapsto
    x^c$ ? En déduire que $\Delta^n f(k)$ est entier et tends vers zéro...}
...
\item Enoncer et démontrer la condition nécessaire d'extremum local (sur un ouvert).

\item Enoncer et démontrer le théorème de Rolle. On admettra la condition nécessaire d'extremum local qui sera démontrée par le numéro 1.

\item Soit $f$ une fonction croissante sur $\mb{R}$, et $(u_n)_{n \ge 0}$ vérifiant $u_{n+1} = f(u_n)$. Que dire quant aux variations de $(u_n)_{n \ge 0}$ ? Et si $f$ est strictement croissante ?

Si $f$ est continue et $(u_n)_{n \ge 0}$ converge, montrer que la limite est un point fixe de $f$.
\end{enumerate}
...
\item  Discuter la continuité et la dérivabilité de  $f_{\alpha}$ en fonction de $\alpha$.

\emph{On complètera d'obscurs calculs par quelques dessins convaincants.}

\item Pour $\alpha > 1$, étudier la régularité aux ordres supérieurs ($\mc{D}^k$, $\mc{C}^k$, $\mc{C}^{\infty}$ ?).
