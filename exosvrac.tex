...
\subsubsection{Manipulation de la structure}

\exercice[Groupe opposé]

Soit $ G := (X, \times, 1)$ un groupe d'ensemble sous-jacent $X$. On définit $G^{\text{op}} := (X, *, 1)$ où $*$ est une loi sur $X$ définie par $x*y = y \times x$.


\begin{enumerate}

\item  Montrer que $G^{\text{op}}$ est un groupe.

\item Soit $\fonction{\phi}{G}{G^{\text{op}}}{x}{x}$. Est-ce en général un morphisme de groupes ?

\item Dans le cas où $G$ est abélien, montrer que $G \simeq G^{\text{op}}$.
\end{enumerate}
