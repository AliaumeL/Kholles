...
\item Décrire la forme des produits cartésiens de 2 intervalles fermés de $\mb{R}$ ? A quoi correspondent-ils dans le plan ?
...
\begin{enumerate}
    ...
    \item Calculer les points où $T_n$ est maximal sur $[-1,1]$
...
Simplifier $\arccos \dfrac{1-x^2}{1+x^2}$ pour $x \in \mb{R}$. On justifiera aussi que ce nombre est défini.

\exercice

Simplifier $\arcsin \dfrac{x}{\sqrt{x^2 +1}}$ pour $x \in \mb{R}$. On justifiera aussi que ce nombre est défini.
...
Soit $f : [0,1] \to [0,1]$ 
On suppose $f$ continue, montrer qu'il existe un 
