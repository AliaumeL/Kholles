...
Soit $f : [a,b] \rightarrow \mb{C}$ une fonction continue telle que $\displaystyle \left|\int_a^b f(t) \d t \right| = \int_a^b |f(t)| \d t \neq 0$.

\begin{enumerate}

\item Soit $\alpha = \int_a^b f(t) \d t \Big/  \left|\int_a^b f(t) \d t \right| $. Justifier qu'il existe $\theta \in \mb{R}$ tel que $\alpha = {\ex}^{\im \theta}$.

\item 

\item Que dire si $\int_a^b |f(t)| \d t = 0$ ?
