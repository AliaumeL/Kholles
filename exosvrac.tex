...
Soit $a<b$. Montrer que $\displaystyle \int_{a}^b \frac{\di t}{\sqrt{(b-t)(t-a)}} = \int_{0}^1 \frac{\di u}{\sqrt{u(1-u)}}$ puis calculer sa valeur.
...
Résoudre sur $\mb{R}$ l'équation différentielle $(x^2+1)y'+2xy+1=0$

\exercice

Soit $f : [a,b] \rightarrow \mb{C}$ une fonction continue telle que $\displaystyle \left|\int_a^b f(t) \di t \right| = \int_a^b |f(t)| \di t \neq 0$.

\begin{enumerate}

\item Soit $\alpha = \int_a^b f(t) \di t \Big/  \left|\int_a^b f(t) \di t \right| $. Justifier qu'il existe $\theta \in \mb{R}$ tel que $\alpha = {\ex}^{\im \theta}$.

\item 

\item Que dire si $\int_a^b |f(t)| \di t = 0$ ?
