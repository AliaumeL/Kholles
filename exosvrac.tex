...
\item Montrer que $\forall n \ge 0, \forall p \ge 1$, $u_{n+p} \le \frac{u_n}{K^p} + \sum_{k=1}^p \frac{v_{n+k}}{K^k}$
...
Soit $0 < u_0 < v_0$, on définit par récurrence pour $n > 0$, $u_{n+1} = \frac{u_n+v_n}{2}$ et $v_{n+1} = \sqrt{u_n v_n}$.

\begin{enumerate}

\item Montrer que $(u_n)_{n \ge 0}$ et $(v_n)_{n \ge 0}$ sont bien définies et que $\forall n \ge 0$, $u_n \le v_n$.
