...
\item Soit $G$ un sous-groupe non trivial de $(\mb{R}, +, 0)$ qui n'est pas dense dans $\mb{R}$.

\begin{enumerate}

\item Montrer que $ \forall x \in \mb{R}$, $\inf G \cap ] x, + \infty[ >x$.

\emph{Indication : sait que c'est vrai pour un $x$ car le sous-groupe n'est pas dense.}

\item Soit $a = \inf G \cap \mb{R}^*_+$. Déduire de ce qui précède que $a > 0$ et qu'il est atteint.

\item Conclure que $G = a \mb{Z}$.
